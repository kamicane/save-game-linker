  ...
  async processGame (gameName, gameObj, argv) {
    ...
    if (exeExists) {
      ...
      try {
        ...
        const iconPNGPath = path.join(argv.iconCacheDir, `${gameName}.png`)

        await fse.writeFile(iconPNGPath, data)
        steamIconPath = iconPNGPath
      } catch (err) {}

      const workingDir = path.isAbsolute(exe) ? path.dirname(exe) : path.join(argv.gamesDir, gameName)

      const steamShortcut = {
        appid: genSteamAppId(gameName),
        appname: gameName,
        Exe: `"${exeFullPath}"`,
        StartDir: `"${path.normalize(workingDir)}"`,
